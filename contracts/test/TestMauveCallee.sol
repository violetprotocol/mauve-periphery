...
contract TestMauveCallee is IMauveSwapCallback {
    ...
    function mauveSwapCallback(
        ...
        if (amount0Delta > 0) {
            IERC20(IMauvePool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(amount0Delta));
        } else {
            assert(amount1Delta > 0);
            IERC20(IMauvePool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(amount1Delta));
        }
    }
}
